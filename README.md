...
> **[📥 點此前往 Releases 下載最新版本](https://github.com/rubujo/InputBox/releases)**
...
可使用任何文字編輯器（如[記事本](https://apps.microsoft.com/detail/9msmlrh6lzf3)、[Visual Studio Code](https://code.visualstudio.com)）開啟 `appsettings.json` 進行編輯。
...
| **控制器 API** | `GamepadProviderType` | 字串 | `"XInput"` | 選擇底層控制器輸入 API：`"XInput"` 或 `"GameInput"`。 |
...
### 4. 使用控制器時，游標或歷史紀錄發生「無限連發」停不下來

這是因為您的手把搖桿可能存在輕微的老化「飄移（Stick Drift）」，錯誤地觸發了程式的連發機制。
請關閉程式後，前往 `%AppData%\InputBox\appsettings.json`，將搖桿死區的閾值大幅調高。例如：
- `"ThumbDeadzoneEnter": 15000`
- `"ThumbDeadzoneExit": 12000`

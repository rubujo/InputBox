...
- **步驟一：叫出輸入框**
  ...
  - 使用前請先啟動輸入框。
- **步驟二：輸入後按遊戲控制器的 A 鍵**
  - 使用 Windows 觸控式鍵盤完成輸入與選字後，先關閉 Windows 觸控式鍵盤，再按下遊戲控制器 A 鍵。
...
- **設定** ⚙️：包含視窗不透明度、回饋強度、遊戲控制器死區與歷程記錄容量等進階設定。
  - **遊戲控制器調校** 🎮：可直接調整搖桿死區、重複輸入延遲與速度；若只是暫時出現飄移或殘留偏移，也可使用「重設目前校正狀態」清除執行期學習值，而不影響已儲存設定。

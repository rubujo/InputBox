...
- **支援不透明度調整** 🌓：可自訂視窗透明度（50% ~ 100%），並支援控制器快速鍵組合快速微調。
...
- **AT 廣播詳細程度控制** 🔇：支援切換螢幕報讀軟體的廣播中斷行為（WCAG 2.2.4）。停用後每則訊息皆完整依序播報，適合需要聆聽完整訊息的使用者（可於右鍵選單切換）。
- **說明對話框** ❓：按下 **F1** 或透過右鍵選單開啟說明視窗，提供鍵盤快速鍵與手把按鍵對應的完整查閱表（WCAG 3.3.5）。
...
- **允許中斷廣播** 🔇：切換螢幕報讀軟體的廣播中斷行為（WCAG 2.2.4）。勾選（預設）時允許新廣播中斷前一則；取消勾選後，每則訊息皆完整依序播報，適合需要聆聽完整訊息的使用者。
- **快速鍵設定** ⚙️：
...
- **說明** ❓：開啟說明對話框，顯示鍵盤快速鍵與手把按鍵對應的完整查閱表（功能等同按下 **F1**）。
...
| | `A11yInterruptEnabled` | 布林值 | `true` | - | 是否允許螢幕報讀廣播中斷前一則訊息（WCAG 2.2.4）。停用後每則訊息皆完整依序播報。 |
...
總結而言，本應用程式之運作邏輯與系統內建之「記事本」或「剪貼簿管理員」本質相同，僅作為文字中轉使用，不具備任何修改、攔截 or 代理目標程式功能之能力。本專案僅設計為解決特定硬體環境下操作不便之一般性輔助應用程式。✨

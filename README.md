...
- **左搖桿**：功能等同鍵盤**方向鍵**。
    - 用於瀏覽輸入歷程記錄或移動游標。
- **右搖桿 ←**／**→**：**虛擬選取模式**。直接擴張或縮減文字選取範圍；快速連續選取時會有更細緻的「拉鏈感」觸覺回饋，按住 **LB／RB** 時可改為單字粒度的選取擴張／縮減。
...
- **右鍵選單導覽**：
  ...
  - **片語子選單開啟時**：可使用 **LB／RB** 快速切換上一頁／下一頁，**按住可連發**；並以 **LT／RT** 直接跳至首頁／末頁，**按住也可連發**。
- **數值輸入對話框操作** 🔢：
  - **方向鍵 ↑／↓**：增加或減少數值。
...
- **片語編輯對話框操作** ✍️：
  - **方向鍵 ↑／↓**：在欄位與按鈕之間切換焦點。
  - **方向鍵 ←／→**：移動游標（或 **LB／RB** + 方向鍵進行單字跳轉）。
  - **右搖桿 左／右**：虛擬選取模式；快速連續選取時會有更細緻的觸覺回饋，按住 **LB／RB** 時可改為單字粒度。
...
- **片語** 📝：展開子選單，顯示已儲存的片語清單（最近插入的片語以 ★ 標示置頂顯示）。點選任一片語即可將其內容插入至文字輸入框。
  ...
  - **分頁行為**：若片語項目超過單頁容量，子選單會顯示 ◀／▶ 分頁控制；每次重新開啟片語子選單時，會回到第 1 頁。控制器可直接使用 **LB／RB** 快速切頁，按住可連續翻頁，並以 **LT／RT** 跳至首頁或末頁；按住時也可持續連發。
- **快速鍵設定** ⚙️：
  ...
  - **快速鍵喚起回饋**：透過全域快速鍵叫出主視窗時，若目前有已連線的控制器，會播放一組簡短的「準備完成」握手震動，方便在螢幕完全切回前就知道已可開始輸入。
- **設定** ⚙️：包含視窗不透明度、回饋強度、遊戲控制器死區、控制器主按鍵配置與歷程記錄容量等進階設定。
  - **遊戲控制器調校** 🎮：可直接調整搖桿死區、重複輸入延遲與速度；若只是暫時出現飄移或殘留偏移，也可使用「重設目前校正狀態」清除執行期學習值，而不影響已儲存設定。若同時連接多支控制器，亦可使用「識別目前遊戲控制器」讓目前作用中的裝置播放特殊震動，方便快速確認是哪一支。

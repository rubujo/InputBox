...
- **最終驗證**：提交前必須執行 `npm test`（Playwright + axe-core A11y）並通過。
- **GPG 簽章提交**：凡執行 Git 提交，預設必須使用有效的 GPG 簽章；若簽章環境異常，應先修復，不得以停用簽章方式繞過。
- **人工驗證補充**：仍需檢查符號全形化、`lang` 屬性、Landmark 導覽結構、四語內容一致性。
